...
extension AnyPlanDescriptor {
  /// Obtains one sample from the ``samples`` that matches the predicate of a
  /// filter.
  ///
  /// - Parameter filter: The filter with which the matching sample will be
  ///   obtained.
...
public protocol SamplePlanFilter: SampleFilter
where Target == AnyPlanDescriptor {}
...
extension SamplePlanFilter where Self == SamplePlanWithGoalsFilter {
  ...
  /// Filter for obtaining the plan with at least one goal matching the given
  /// filter.
  ///
  /// - Parameter goals: Filter serving as a predicate which a goal of the plan
  ///   should match in order for that plan to be the resulting one.
  public static func withGoals(_ goals: any SampleGoalFilter) -> Self {
    .init(goals)
  }
}
